...
const getProductCategoryList = async () => {
  const res = await request.get({ url: 'api/v1/product-category/tree' })
...
const createData = async () => {
  ...
  if (formData) {
    const res = await request.post({ url: 'api/v1/product-category', data: formData })
    if (res) {
...
const handleCreate = (row: any) => {
  temp.parentPcid = row ? row.pcid : 0
  dialogVisible.value = true
  actionType.value = 'create'
}
const handleDelete = async (_: any, row: any) => {
  const res = await request.delete({ url: 'api/v1/product-category/' + row.pcid })
  if (res) {
    await getProductCategoryList()
...
const handleDisable = async (row: any) => {
  const res = await request.put({
    url: 'api/v1/product-category/' + row.pcid,
    data: { deleted: 1 }
  })
  if (res) {
    await getProductCategoryList()
